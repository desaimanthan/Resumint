...
export function VoiceInterviewAgent({
  ...
  // Track current AI response for building transcript
    ...
    switch (event.type) {
      ...
      case 'response.audio_transcript.delta':
        ...
        if (aiText) {
          ...
          currentAiResponseRef.current += aiText
          
          setTranscript(prev => {
            const lines = prev.split('\n\n')
            const lastLine = lines[lines.length - 1]
            if (lastLine && lastLine.startsWith('🤖 **AI Interviewer:**')) {
              lines[lines.length - 1] = '🤖 **AI Interviewer:** ' + currentAiResponseRef.current
            } else {
              const separator = prev ? '\n\n' : ''
              lines.push(separator + '🤖 **AI Interviewer:** ' + currentAiResponseRef.current)
            }
      ...
      case 'response.audio_transcript.done':
        // AI response complete - reset for next response
        console.log('AI transcript complete:', currentAiResponseRef.current)
        currentAiResponseRef.current = ''
        break

      case 'response.text.delta':
        // Alternative AI text event - some models use this instead
        const altAiText = event.delta
        if (altAiText) {
          console.log('AI text delta (alternative):', altAiText)
          currentAiResponseRef.current += altAiText
      ...
      case 'response.text.done':
        console.log('AI text complete (alternative):', currentAiResponseRef.current)
        currentAiResponseRef.current = ''
        break
